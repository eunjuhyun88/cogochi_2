...
- Publish tags: `chatbattle, context-kit`
...
| `ctx:autopilot` | ctx | `node scripts/dev/context-autopilot.mjs` |
...
| `docs:refresh` | docs | `node scripts/dev/refresh-generated-context.mjs && node scripts/dev/refresh-context-metrics.mjs && node scripts/dev/refresh-task-contract-report.mjs` |
| `docs:refresh:check` | docs | `node scripts/dev/refresh-generated-context.mjs --check && node scripts/dev/refresh-context-metrics.mjs --check && node scripts/dev/refresh-task-contract-report.mjs --check` |
...
| `eval:validate` | eval | `node scripts/dev/refresh-context-validation-report.mjs` |
| `gate:context` | gate | `npm run docs:check && npm run ctx:check -- --strict && npm run contract:check && npm run coord:check && npm run sandbox:check` |
...
| `safe:split` | safe | `bash scripts/dev/split-wip-slice.sh` |

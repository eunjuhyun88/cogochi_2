...
For the release-facing front door and player-flow restructuring, also read [STEAM_RELEASE_REFRAME.md](/Users/ej/Downloads/maxidoge-clones/Cogochi/docs/STEAM_RELEASE_REFRAME.md).
...
Cogochi is a squad-based AI agent raising game where the player recruits named agents, decides how each one grows, trains them on signal patterns, assembles them into a mission squad, and sends them into a chart-shaped battlefield where market pressure becomes visible combat.
...
`pick agents` -> `shape growth` -> `train` -> `battle` -> `reflect` -> `promote`
...
1. choosing which agents to recruit or field
2. choosing what each agent should learn next
3. choosing what an agent sees
4. choosing how it interprets what it sees
5. choosing how it remembers
6. choosing how four agents work together
7. choosing when to intervene during battle
8. interpreting the result and improving the agents
...
`discover candidates` -> `build a bench` -> `specialize a squad` -> `clear harder scenarios and whale bosses` -> `promote prestige agents` -> `unlock rental demand`

### 5.2 Session loop

`scan the recruitment wall` -> `pin several agents` -> `choose one growth lane per agent` -> `run one short mission` -> `watch what happened` -> `collect lessons` -> `queue one improvement`
...
The first screen must make agent choice feel exciting.

This means:

- the command deck comes before the world map
- multiple interesting agent candidates must be visible at once
- multi-select happens before deep editing
- the player should reach a first battle quickly, but only after choosing who they want to grow
